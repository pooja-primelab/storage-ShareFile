/*
	...
	saveFile():
		- Private function that Peers use to save a file to 'disk' once obtained from
		  another Peer.
	shareFile():
		- when peer register any new file, share that file to all other peers.
*/
...
/*
	Share File to all other peers as soon as we any peer register that file to Master
*/
func (p *Peer) ShareFile(fileName string) error {
	p.mu.Lock()
	defer p.mu.Unlock()
